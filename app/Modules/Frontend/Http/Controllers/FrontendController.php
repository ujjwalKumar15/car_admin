...
//    {
    ...
    public function price_filter(Request $request)
    {
        ...
        if (isset($request->category) && isset($request->color)) {
            $products = Product::whereIn('brandid', $request->category)->whereIn('colorid', $request->color)
         ...
         elseif (isset($request->color)) {
            ...
            $products = Product::where('status','Y')->whereBetween('price', [(int)$request->minimum, (int)$request->maximum])->orderBy($request->sort_by, $request->order_by)->get();
        }
